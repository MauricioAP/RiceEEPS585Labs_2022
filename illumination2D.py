...
if len(sys.argv) < 2:
    ...
    print(' Usage: > python3 illumination2D.py synthmodel2D or SEGsaltmodel')
    print( ' Example: > python3 illumination2D.py synthmodel2D' )
    print()
    exit()
...
if multishot:
    ...
    if exists("./acquisition_plan.npy"):
        sloc = np.load("./acquisition_plan.npy")
        sloc = sloc.astype(np.int32)
        ns = sloc.shape[0]
    else:
        ns = 5
        # shots locations
